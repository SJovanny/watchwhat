...
export default function MoviesPage() {
  ...
  const handleTabChange = (tab: 'popular' | 'top_rated' | 'now_playing' | 'upcoming') => {
    setActiveTab(tab);
    setCurrentPage(1);
    setSearchQuery('');
    setIsSearching(false);
  ...
  const getTabLabel = (tab: string) => {
    switch (tab) {
      ...
      case 'now_playing': return 'En salles';
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Films</h1>
          ...
          <div className="mt-8 border-b border-gray-200 dark:border-gray-700">
            <nav className="-mb-px flex space-x-8 overflow-x-auto">
              {(['popular', 'top_rated', 'now_playing', 'upcoming'] as const).map((tab) => {
        ...
        {isSearching && searchQuery && (
          <div className="mb-6">
            <div className="flex items-center space-x-2 text-gray-600 dark:text-gray-400">
              <Search size={16} />
              <span>
                Résultats pour "{searchQuery}" : {searchResults.length} film{searchResults.length > 1 ? 's' : ''} trouvé{searchResults.length > 1 ? 's' : ''}
              </span>
            </div>
          </div>
        )}
