    ...
    const loadMovieData = async () => {
      try {
        ...
        const trailer = videosData.results.find(
          (video: Video) => video.type === 'Trailer' && video.site === 'YouTube' && video.official
        ) || videosData.results.find(
          (video: Video) => video.type === 'Teaser' && video.site === 'YouTube'
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div className="relative h-[60vh] bg-gradient-to-br from-slate-900 via-gray-900 to-zinc-900 flex items-center justify-center">
          <div className="text-center">
            ...
            <h3 className="text-white text-2xl font-bold mb-3">Chargement du film</h3>
            <p className="text-white/70 text-lg">Préparation de votre expérience cinéma...</p>
          </div>
        </div>
            ...
            <div className="max-w-7xl mx-auto">
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1 text-white">
                  ...
                  {movie.tagline && (
                    <p className="text-lg text-gray-200 mb-4 italic">"{movie.tagline}"</p>
                  )}

                  {/* Métadonnées */}
                  <div className="flex flex-wrap items-center gap-4 mb-4 text-sm">
                    <div className="flex items-center gap-1">
                      <Star size={16} className={getRatingColor(movie.vote_average)} fill="currentColor" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {movie.credits?.crew && movie.credits.crew.length > 0 && (
              <section>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Équipe technique</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {movie.credits.crew
                    .filter(person => ['Director', 'Producer', 'Screenplay', 'Story', 'Writer'].includes(person.job))
                    .slice(0, 8)
                    .map((person: Crew) => (
                    <div key={`${person.id}-${person.job}`} className="text-center">
                      ...
                      <p className="font-semibold text-sm text-gray-900 dark:text-white">{person.name}</p>
                      <p className="text-xs text-gray-600 dark:text-gray-400">{person.job}</p>
                    </div>
                  ))}
                </div>
              </section>
            ...
            {similarMovies.length > 0 && (
              <section>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">Films similaires</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-md">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div>
                  <dt className="font-medium text-gray-600 dark:text-gray-400">Date de sortie</dt>
                ...
                <div>
                  <dt className="font-medium text-gray-600 dark:text-gray-400">Durée</dt>
                  <dd className="text-gray-900 dark:text-white">{formatRuntime(movie.runtime)}</dd>
                </div>

                {movie.budget > 0 && (
                  <div>
                    <dt className="font-medium text-gray-600 dark:text-gray-400">Budget</dt>
                    <dd className="text-gray-900 dark:text-white">{formatCurrency(movie.budget)}</dd>
                  </div>
                )}

                {movie.revenue > 0 && (
                  <div>
                    <dt className="font-medium text-gray-600 dark:text-gray-400">Recettes</dt>
                    <dd className="text-gray-900 dark:text-white">{formatCurrency(movie.revenue)}</dd>
                  </div>
                )}

                {movie.spoken_languages && movie.spoken_languages.length > 0 && (
                  <div>
                    <dt className="font-medium text-gray-600 dark:text-gray-400">Langues</dt>
                    <dd className="text-gray-900 dark:text-white">
                ...
                {movie.imdb_id && (
                  <div>
                    <dt className="font-medium text-gray-600 dark:text-gray-400">IMDB</dt>
                    <dd>
                      <a
          ...
          <div className="relative w-full max-w-5xl mx-auto">
            ...
            >
              <div className="relative">
                <div className="w-12 h-12 bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl flex items-center justify-center group-hover:bg-red-500/20 group-hover:border-red-500/40 transition-all duration-300">
                  <svg className="h-6 w-6 text-white group-hover:text-red-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="mt-6 text-center">
              ...
              <p className="text-white/70">
                Bande-annonce officielle • {formatDateToYear(movie.release_date)}
              </p>
            </div>
          </div>
        </div>
